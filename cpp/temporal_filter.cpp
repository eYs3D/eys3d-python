...
// Portable 8-bit popcount; the operand is always the uint8_t history
// mask, so a two-step parallel bit count suffices and stays
// compiler-neutral.
...
inline bool persistence_pattern_matches(uint8_t hist, int level) {
    switch (level) {
        case 1: return popcount8(hist) == 8;                     // 8/8
        case 2: return popcount8(hist & 0x07) >= 2;              // 2/last 3
        case 3: return popcount8(hist & 0x0F) >= 2;              // 2/last 4
        case 4: return popcount8(hist) >= 2;                     // 2/8
...
void temporal_filter_apply(uint16_t* __restrict cur, int w, int h,
                           TemporalState& state,
                           const TemporalFilterParams& params)
{
    ...
    uint16_t* __restrict const prev_buf = state.prev.data();
    uint8_t*  __restrict const hist_buf = state.valid_history.data();
    ...
    for (int v = 0; v < h; ++v) {
        ...
        uint16_t* __restrict cur_row  = cur   + row;
        uint16_t* __restrict prev_row = prev_buf + row;
        uint8_t*  __restrict hist_row = hist_buf + row;
