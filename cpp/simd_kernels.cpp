...
// Portable scalar row converter — the reference implementation and the
// non-aarch64 fallback. The +128 bias on (Y << 8) makes the final >> 8
...
uint32_t count_valid_depth(const uint16_t* row, int w) {
    ...
    // Per-lane hit counts accumulate in uint16 lanes; drain the
    // accumulator well before a lane could reach 65535 hits.
