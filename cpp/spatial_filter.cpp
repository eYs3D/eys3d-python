...
//   * a hole following an initialised lane is filled with prev while
//     fill_run is below max_fill; max_fill <= 0 disables bridging;
...
void disparity_promote_to_q4(const uint16_t* __restrict raw,
                             uint16_t* __restrict out_q4,
                             int w, int h) {
#ifdef __aarch64__
    ...
    #pragma omp parallel for schedule(static)
    for (int v = 0; v < h; ++v) {
        const uint16_t* __restrict p = raw   + static_cast<size_t>(v) * w;
        uint16_t*       __restrict q = out_q4 + static_cast<size_t>(v) * w;
        int u = 0;
#ifdef __aarch64__
