...
// decided by the caller; the capture engine always feeds Q4-promoted
// D11 disparity, so delta keeps one meaning whether or not the spatial
...
struct TemporalFilterParams {
    ...
    // Units match the caller's buffer (Q4 disparity in the capture
    // engine: raw disparity << 4). A gap at or above delta is treated
    // as a depth edge and the blend is skipped.
