...
void fill_around_impl(uint16_t* __restrict dst,
    ...
    for (int v = 0; v < h; ++v) {
        ...
        uint16_t* __restrict row = dst + static_cast<size_t>(v) * w;
        for (int u = 0; u < w; ++u) {
            ...
            for (int nv = v_lo; nv <= v_hi; ++nv) {
                const uint16_t* __restrict src_row = src + static_cast<size_t>(nv) * w;
...
void hole_fill_z(uint16_t* __restrict z_mm, int w, int h,
                 HoleFillMode mode,
                 int left_skip,
