...
/*! \fn int APC_GetFWFSLength(
	...
	\param pid	(optional) encoded pid to select target GRAPE path.
		Use APC_PID_GRAPE_PATH1 or APC_PID_GRAPE_MONO to query a slave path's FWFS.
		Defaults to APC_PID_DEFAULT_MASTER (master / non-GRAPE devices).
	\return success:APC_OK, others:see eSPDI_ErrCode.h
*/
int APC_API APC_GetFWFSLength(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, int iFWFSIndex, int& iFWFSLength, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_GetFileData(
	...
	\param pid	(optional) encoded pid to select target GRAPE path.
		Use APC_PID_GRAPE_PATH1 or APC_PID_GRAPE_MONO to read a slave path's flash file.
		Defaults to APC_PID_DEFAULT_MASTER (master / non-GRAPE devices).
	\return success:APC_OK, others:see eSPDI_ErrCode.h
*/
int APC_API APC_GetFileData(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, int nID, int DataSize, BYTE *lpData, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_GetYOffset(
	...
	\param pActualLength	actual byte of reading
	\param index	index of Y offset file ID
	\param pid	product id (can be encoded for grape devices using APC_PID_GRAPE_PATH0/PATH1/MONO), defaults to EOF (master device)
	\return success:APC_OK, others:see eSPDI_ErrCode.h
*/
int APC_API APC_GetYOffset(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);
...
/*!	\fn APC_GetRectifyTable(
	...
	\return success:APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetRectifyTable(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_GetZDTable(
	...
	\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetZDTable(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, PZDTABLEINFO pZDTableInfo, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_SetYOffset(
	...
	\return success:APC_OK, others:see eSPDI_ErrCode.h
*/
int APC_API APC_SetYOffset(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_SetYOffset_Advanced(
    ...
    \param int pid	product id (can be encoded for grape devices using APC_PID_GRAPE_PATH0/PATH1/MONO), defaults to EOF (master device)
    \return success:APC_OK, others:see eSPDI_ErrCode.h
*/
int APC_API APC_SetYOffset_Advanced(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_SetRectifyTable(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid)
	\brief set rectify data to flash
	\param pHandleApcDI	 the pointer to the initilized ApcDI SDK instance
	\param pDevSelInfo	pointer of device select index
	\param buffer	buffer containing rectify table data
	\param BufferLength	input buffer length
	\param pActualLength	actual length written
	\param index	index to identify rectify table for corresponding depth
	\param pid	product id (can be encoded for grape devices using APC_PID_GRAPE_PATH0/PATH1/MONO), defaults to EOF (master device)
	\return success:APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_SetRectifyTable(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);

/*! \fn int APC_SetRectifyTable_Advanced(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid)
    \brief set rectify data to flash in both Groups #1 and #2 when the firmware has the flash protection.
	\param pHandleApcDI	 the pointer to the initilized ApcDI SDK instance
	\param pDevSelInfo	pointer of device select index
	\param buffer	buffer containing rectify table data
	\param BufferLength	input buffer length
	...
	\return success:APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_SetRectifyTable_Advanced(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);

/*! \fn APC_SetZDTable(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, PZDTABLEINFO pZDTableInfo, int pid)
	\brief set disparity and Z values to flash, see APC_GetZDTable except get
	\param int pid	product id (can be encoded for grape devices using APC_PID_GRAPE_PATH0/PATH1/MONO), defaults to EOF (master device)
*/
int APC_API APC_SetZDTable(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, PZDTABLEINFO pZDTableInfo, int pid = APC_PID_DEFAULT_MASTER);

/*! \fn APC_SetZDTable_Advanced(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, PZDTABLEINFO pZDTableInfo, int pid)
    \brief set disparity and Z values to flash, see APC_GetZDTable except get
	\param int pid	product id (can be encoded for grape devices using APC_PID_GRAPE_PATH0/PATH1/MONO), defaults to EOF (master device)
*/
int APC_API APC_SetZDTable_Advanced(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, PZDTABLEINFO pZDTableInfo, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_GetRectifyMatLogData(
	...
	\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetRectifyMatLogData(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, eSPCtrl_RectLogData *pData, int index, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_EnableTimingCount(
void *pHandleApcDI,
PDEVSELINFO pDevSelInfo,
bool enable)
\brief enable or disable G120 timing counter mode for module sync
\param pHandleApcDI	 the pointer to the initilized ApcDI SDK instance
\param pDevSelInfo	pointer of device select index
\param enable	set true to enable timing counter mode, or set false to disable it
\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_EnableTimingCount( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, bool enable );
...
\brief Query whether the device has Master capability for Module Sync.
       Master capability is determined by FW hardware design (0xF7 bit[2], R/O).
       SW reads this to decide whether to enable the "Master" UI checkbox,
       allowing the user to confirm this device as Master and trigger Force Sync.
       Currently only supported on G120 (GRAPE) devices.
       Must be called after PATH1 is streaming.
\param pHandleApcDI	 the pointer to the initialized ApcDI SDK instance
\param pDevSelInfo	pointer of device select index
\param pbCanBeMaster	output: TRUE if FW reports this device has Master hardware capability
\return success: APC_OK, APC_DEVICE_NOT_SUPPORT for non-GRAPE devices, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetModuleSyncCapability( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BOOL *pbCanBeMaster );

/*! \fn int APC_SetModuleSyncForceSync(
void *pHandleApcDI,
PDEVSELINFO pDevSelInfo,
bool bEnable)
\brief Trigger or stop Force Sync on a device confirmed as Master.
       Must be called only after APC_GetModuleSyncCapability confirms Master capability
       and the user has confirmed this device as Master via the UI.
       Controls FW register 0xF7 bit[1] via PATH1.
       Currently only supported on G120 (GRAPE) devices.
       Must be called while PATH1 is streaming.
\param pHandleApcDI	 the pointer to the initialized ApcDI SDK instance
\param pDevSelInfo	pointer of device select index
\param bEnable	TRUE: trigger Force Sync (this device acts as Master); FALSE: stop Force Sync
\return success: APC_OK, APC_DEVICE_NOT_SUPPORT for non-GRAPE devices, others: see eSPDI_ErrCode.h
*/
int APC_API APC_SetModuleSyncForceSync( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, bool bEnable );

/*! \fn int APC_GetFWRegisterEx(
void *pHandleApcDI,
PDEVSELINFO pDevSelInfo,
...
int flag,
int pid)
\brief get firmware register value, with device path selection via EncodedPID
\param pHandleApcDI	 the pointer to the initilized ApcDI SDK instance
\param pDevSelInfo	pointer of device select index
\param address	register address
\param pValue	pointer of value got from register address
\param flag	address and value data length (e.g. FG_Address_1Byte | FG_Value_1Byte)
\param pid	EncodedPID to select device path (e.g. APC_PID_GRAPE_PATH1 for GRAPE FW registers)
\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetFWRegisterEx( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, unsigned short address, unsigned short *pValue, int flag, int pid );

/*! \fn int APC_SetFWRegisterEx(
void *pHandleApcDI,
PDEVSELINFO pDevSelInfo,
unsigned short address,
...
int flag,
int pid)
\brief set firmware register value, with device path selection via EncodedPID
\param pHandleApcDI	 the pointer to the initilized ApcDI SDK instance
\param pDevSelInfo	pointer of device select index
\param address	register address
...
\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_SetFWRegisterEx( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, unsigned short address, unsigned short nValue, int flag, int pid );
...
int APC_API APC_GetPointCloudForMultibaseline( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, unsigned char* ImgColor, int CW, int CH,
                                                                                      unsigned char* ImgDepth, int DW, int DH,
                                                                                      unsigned char* depthIndex, std::vector<PointCloudInfo>& grapePointCloudInfo,
                                                                                      unsigned char* pPointCloudRGB, float* pPointCloudXYZ, float Near, float Far );
