...
/* APC PID */
#define APC_PID_DEFAULT_MASTER  (-1)  // Sentinel: use master device; do not encode a path
...
#define APC_PID_80363C      APC_PID_GRAPE
#define APC_PID_80363IR     0x0211

// Encoded grape PIDs for different device paths
#define APC_PID_GRAPE_PATH0    0x0202  // Same as original (master device)
#define APC_PID_GRAPE_PATH1    0x1202  // PATH1 device (0x1000 | 0x0202)
#define APC_PID_GRAPE_MONO     0x2202  // MONO device (0x2000 | 0x0202)

#define APC_PID_80363IR_PATH0  0x0211  // Same as original (master device)  
#define APC_PID_80363IR_PATH1  0x1211  // PATH1 device (0x1000 | 0x0211)
#define APC_PID_80363IR_MONO   0x2211  // MONO device (0x2000 | 0x0211)

// Helper macros for grape device handling
#define IS_GRAPE_PID(pid) \
    ((pid & 0x0FFF) == APC_PID_GRAPE || (pid & 0x0FFF) == APC_PID_80363IR)

#define GET_BASE_GRAPE_PID(pid) (pid & 0x0FFF)

#define GET_GRAPE_DEVICE_TYPE(pid) \
    (IS_GRAPE_PID(pid) ? ((pid & 0xF000) >> 12) : GRAPE_PATH0)

// Encoding constants
#define GRAPE_DEVICE_PATH0_ENCODING  0x0000  // 0 << 12
#define GRAPE_DEVICE_PATH1_ENCODING  0x1000  // 1 << 12  
#define GRAPE_DEVICE_MONO_ENCODING   0x2000  // 2 << 12

// GRAPE Flash Write Protection Constants
#define PLUGIN_GRAPE_BASE_FLASH_ADDR           (0x20000)
#define GRAPE_PLUGIN_SIGNATURE                 "GrapeCfg"
#define GRAPE_PLUGIN_SIGNATURE_SIZE            (8)
#define PLUGIN_GRAPE_UNP_OFFSET_OFFSET         (0x2C)
#define GRAPE_BOOT_OPT5_ADDR                   (0x9FA6)
#define GRAPE_WP_GPIO_DISABLED                 (0x00)

// GRAPE GPIO Register Mapping for Write Protection
#define GRAPE_GPIO1X_DATA_REG                  (0xF020)
#define GRAPE_GPIO1X_CTRL_REG                  (0xF021)
#define GRAPE_GPIO2X_DATA_REG                  (0xF023)
#define GRAPE_GPIO2X_CTRL_REG                  (0xF024)
#define GRAPE_GPIO3X_DATA_REG                  (0xF026)
#define GRAPE_GPIO3X_CTRL_REG                  (0xF027)
#define GRAPE_GPIO4X_DATA_REG                  (0xF029)
#define GRAPE_GPIO4X_CTRL_REG                  (0xF02A)
#define GRAPE_UART_OFF_REG                     (0xF01C)
#define GRAPE_UART_OFF_BIT                     (0x04)
...
// Grape Device Name =============================================
#define    GRAPE_DEVICE_PATH0  "HD Depth Camera:vid_3438"
#define    GRAPE_DEVICE_PATH1  "Camera_1"
#define    GRAPE_DEVICE_MONO   "Camera_mono"
//================================================================
...
struct APCImageType
{
    ...
    {
        return (type == COLOR_YUY2 || type == COLOR_RGB24 || type == COLOR_MJPG || type == COLOR_Y12Bits || type == COLOR_MJPG_12BITS);
    }

    static bool IsImagePLY(APCImageType::Value type)
    {
        return (type == PLY_FIXED || type == PLY_VARIABLE);
    }

    static bool IsImageDepth(APCImageType::Value type)
    {
        return (type != IMAGE_UNKNOWN && !IsImageColor(type) && !IsImagePLY(type));
    }
...
/*! \fn int APC_GetSerialNumber( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, char *pSerialNum, int nBufferSize, int *pActualLength, int pid)
	...
	\param pid	(optional) encoded pid to select target GRAPE path.
		Each GRAPE path (PATH0/PATH1/MONO) stores its own serial number independently.
		Use APC_PID_GRAPE_PATH1 or APC_PID_GRAPE_MONO to read a slave path's SN.
		Defaults to APC_PID_DEFAULT_MASTER (master / non-GRAPE devices).
	\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetSerialNumber( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *pSerialNum, int nBufferSize, int *pACtualSNLenByByte, int pid = APC_PID_DEFAULT_MASTER);

/*! \fn int APC_SetSerialNumber( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, char *pSerialNum, int nBufferSize, int pid)
    ...
    \param pid	(optional) encoded pid to select target GRAPE path.
		Each GRAPE path (PATH0/PATH1/MONO) stores its own serial number independently.
		Use APC_PID_GRAPE_PATH1 or APC_PID_GRAPE_MONO to write a slave path's SN.
		Defaults to APC_PID_DEFAULT_MASTER (master / non-GRAPE devices).
    \return success: EtronDI_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_SetSerialNumber(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *pSerialNum, int nBufferSize, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_GetLogData(
	...
	\param pActualLength	actual length has written to buffer
	\param index	index to identify log data for corresponding depth
	\param pid	encoded PID for multi-path devices (e.g., APC_PID_GRAPE_PATH1), use EOF for master device
	\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetLogData      ( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_SetLogData(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid)
	\brief set log data to flash
	\param pHandleApcDI	 the pointer to the initilized ApcDI SDK instance
	\param pDevSelInfo	pointer of device select index
	\param buffer	buffer to store log data
	...
	\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_SetLogData      ( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);

/*! \fn int APC_SetLogData_Advanced(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid)
    \brief set log data to flash
    \param pHandleApcDI	 the pointer to the initilized ApcDI SDK instance
    \param pDevSelInfo	pointer of device select index
    \param buffer	buffer to store log data
    ...
    \return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_SetLogData_Advanced(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, BYTE *buffer, int BufferLength, int *pActualLength, int index, int pid = APC_PID_DEFAULT_MASTER);
...
/*! \fn int APC_OpenDevice(
	...
	\param pHandleApcDI   the pointer to the initilized ApcDI SDK instance
	\param pDevSelInfo    pointer of device select index
	\param iFps           desired frame rate
	\param callbackFn     image callback function
	\param pCallbackParam user data passed to the callback
	\param pid            device PID. Use EOF (-1) for the legacy master device.
	                      For GRAPE devices, pass an encoded PID (APC_PID_GRAPE_PATH0 /
	                      APC_PID_GRAPE_PATH1 / APC_PID_GRAPE_MONO) to select the target PATH.

	\par Legacy devices (pid == EOF)
	\param colorStreamIndex  index of the desired color stream (from APC_GetDeviceResolutionList)
	\param depthStreamIndex  index of the desired depth stream (from APC_GetDeviceResolutionList)
	\param depthStreamSwitch depth output control bitmask:
	<table>
	<caption id="multi_row">Image Control Mode</caption>
	<tr><th>Value <th>Description
	<tr><td>0x01  <td>color and depth frame output synchronously, for depth map module only
	...
	\par GRAPE devices (pid == APC_PID_GRAPE_PATH0 / PATH1 / MONO)
	The three stream parameters are reinterpreted as follows:
	<table>
	<tr><th>Parameter        <th>GRAPE meaning
	<tr><td>colorStreamIndex <td>Not used for routing (routing is done via encoded pid). Pass GRAPE_PATH0/PATH1/MONO for readability; only validated to be in range 0-2.
	<tr><td>depthStreamIndex <td>Stream index into the array returned by APC_GetDeviceResolutionListEx with the same encoded PID. Find the index by matching the desired resolution against that array.
	<tr><td>depthStreamSwitch<td>APCImageType::Value — the image data type for this PATH (e.g. APCImageType::COLOR_YUY2, APCImageType::DEPTH_11BITS).
	</table>

	Call APC_SetDepthDataTypeEx() with the target PATH's encoded PID before calling this function
	to configure FW register 0xF0 (output format). Open PATH0, PATH1, and MONO with separate calls.

	\par GRAPE example
	\code
	// 1. Get PATH1 stream list
	APC_STREAM_INFO streamInfo[APC_MAX_STREAM_COUNT];
	int count = APC_GetDeviceResolutionListEx(handle, &sel,
	    APC_MAX_STREAM_COUNT, streamInfo, 0, nullptr, APC_PID_GRAPE_PATH1) >> 8;

	// 2. Find the index matching the desired resolution (e.g. 2560x720 YUYV)
	int streamIndex = -1;
	for (int i = 0; i < count; i++) {
	    if (streamInfo[i].nWidth == 2560 && streamInfo[i].nHeight == 720
	            && !streamInfo[i].bFormatMJPG) {
	        streamIndex = i; break;
	    }
	}

	// 3. Configure FW 0xF0 output format
	APC_SetDepthDataTypeEx(handle, &sel, videoMode, APC_PID_GRAPE_PATH1);

	// 4. Open PATH1
	APC_OpenDevice(handle, &sel,
	    GRAPE_PATH1,               // colorStreamIndex: not used, pass PATH constant for clarity
	    streamIndex,               // depthStreamIndex: index from step 2
	    APCImageType::COLOR_YUY2,  // depthStreamSwitch: APCImageType of this PATH
	    fps, callback, param, APC_PID_GRAPE_PATH1);
	\endcode
...
/*! \fn int APC_GetSerialNumberFromLog( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, char *pSerialNum, int nBufferSize, int *pActualLength, int pid)
	\brief get the module serial number from calibration log data
	\param pHandleApcDI	CApcDI handler
	\param pDevSelInfo	pointer of device select index
	\param pSerialNum	a pre-allocated buffer to store the serial number.
	\param nBufferSize	length of pSerialNum buffer
	\param pActualLength	actual number of bytes copied to the buffer
	\param pid	(optional) encoded pid to select target GRAPE path.
		Each GRAPE path (PATH0/PATH1/MONO) stores its own log data independently.
		Use APC_PID_GRAPE_PATH1 or APC_PID_GRAPE_MONO to read a slave path's log SN.
		Defaults to APC_PID_DEFAULT_MASTER (master / non-GRAPE devices).
	\return success: APC_OK, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetSerialNumberFromLog( void *pHandleApcDI, PDEVSELINFO pDevSelInfo, char *pSerialNum, int nBufferSize, int *pActualLength, int pid = APC_PID_DEFAULT_MASTER);
...
typedef enum
{
    ...
    APC_SENSOR_TYPE_H68,
    APC_SENSOR_TYPE_OG02B10,
    APC_SENSOR_TYPE_OG02B1B,
...
Sets cipher on master ApcDI and all slave instances.
*/
...
/*! \enum APC_THERMAL_SOURCE
    \brief Selects which temperature sensor to read in APC_GetThermalSensorTemperature.
*/
typedef enum {
    APC_THERMAL_SOURCE_DEFAULT = 0, /**< Ambient/display sensor (TI chip for 80362/TARYN/8077; OG02B10 for GRAPE) */
    APC_THERMAL_SOURCE_AMBIENT = 1, /**< Same as DEFAULT — external ambient temperature chip */
    APC_THERMAL_SOURCE_IMAGE   = 2, /**< Image sensor built-in temperature register (for calibration compensation) */
} APC_THERMAL_SOURCE;

/*! \fn int APC_GetThermalSensorTemperature(
        void *pHandleApcDI,
        PDEVSELINFO pDevSelInfo,
        float *pfTemperature,
        APC_THERMAL_SOURCE source)
    \brief Get temperature (in Celsius) from the device's thermal sensor.
        APC_THERMAL_SOURCE_DEFAULT / APC_THERMAL_SOURCE_AMBIENT:
          - GRAPE / YX80363IR : OG02B10 image sensor (registers 0x4417/0x4418)
          - 80362 / TARYN / 8077 : External TI thermal chip (register 0x00, slave 0x90)
        APC_THERMAL_SOURCE_IMAGE (image sensor built-in, for calibration compensation):
          - GRAPE / YX80363IR : OG02B10 (same as DEFAULT)
          - 80362 / IRIS : ONSEMI AR0144 image sensor
          - IVY2 : STM VD5X slave sensor
    \param pHandleApcDI  CApcDI handler
    \param pDevSelInfo   pointer of device select index
    \param pfTemperature pointer to float receiving temperature in degrees Celsius
    \param source        which sensor to read; use APC_THERMAL_SOURCE_DEFAULT for existing behavior
    \return success: APC_OK, unsupported device: APC_NotSupport, others: see eSPDI_ErrCode.h
*/
int APC_API APC_GetThermalSensorTemperature(void *pHandleApcDI, PDEVSELINFO pDevSelInfo, float *pfTemperature, APC_THERMAL_SOURCE source);
